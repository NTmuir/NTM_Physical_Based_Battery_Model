...
@show result = optimize(objective_function,[lb lb; ub ub],NSGA2(options=Options(debug = true,store_convergence = true,iterations=1000,time_limit = 120.0)))
...
animation = @animate for i in 1:length(result.convergence)
    ...
    scatter!(p[1], X[:,1], X[:,2], label="", xlim=(0, ub), ylim=(0,ub), title="Population")
